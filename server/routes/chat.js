...
Learning Objectives: ${(lesson.learningObjectives || []).join('; ') || 'Explain the lesson clearly and check understanding.'}
${lesson.exercise ? `Interactive Exercise:
Prompt: ${lesson.exercise.prompt}
Starter code or data: ${lesson.exercise.starterCode}
Expected outcome: ${lesson.exercise.expectedOutcome}` : ''}
${lesson.challenge ? `Practical Challenge: ${lesson.challenge}` : ''}
${lesson.hints?.length ? `Hint ladder (reveal one hint at a time): ${lesson.hints.map((hint, i) => `${i + 1}. ${hint}`).join(' ')}` : ''}
${lesson.labType ? `Lab scope: This is a controlled Cyber Academy ${lesson.labType} lab. Keep all activity inside the supplied sandbox and never suggest testing real systems.` : ''}
...
- Teach progressively: explain one concept, ask the learner to apply it, inspect their response, and only then introduce the next step
- For lessons with an interactive exercise, give the learner the prompt and starter code, wait for their attempt, then review it instead of giving the completed answer immediately
- For practical challenges, offer hints one at a time and explain mistakes; do not reveal the answer or flag until the learner has made a reasonable attempt
- When the learner completes an exercise or challenge successfully, celebrate it and recommend the next lesson; mention the lesson XP reward when one is defined
- For coding and lab lessons, keep all examples defensive and authorized. Never provide instructions for attacking real systems, evading detection, stealing credentials, or deploying malware
...
router.post('/message', requireAuth, async (req, res) => {
  ...
  // Build conversation history (last 40 messages) — user message NOT yet persisted
  ...
  const messages = [
    ...
    // Append the new user message for the AI call without persisting it yet
  ...
  try {
    ...
    // Persist user + assistant messages as an atomic pair using a MongoDB transaction.
    // If either write fails, the session aborts and neither message is stored,
    // so the client can retry without creating duplicates or orphaned messages.
    const session = await mongoose.startSession();
    try {
      await session.withTransaction(async () => {
        await Message.create(
          [{ userId: req.session.userId, lessonId, role: 'user', content: content.trim() }],
          { session }
        );
        await Message.create(
          [{ userId: req.session.userId, lessonId, role: 'assistant', content: reply }],
          { session }
        );
      });
    } finally {
    ...
    // Neither message is persisted on any failure path — safe to retry
    if (err.message?.includes('Transaction')) {
      res.status(500).json({ error: 'Failed to save messages. Please try again.' });
    } else {
